...
func (a *adapter) start(ctx context.Context) {
	...
	grpcRetryOptions := grpc_retry.UnaryClientInterceptor(grpc_retry.WithMax(a.config.MaxRetries), grpc_retry.WithPerRetryTimeout(a.config.RPCTimeout), grpc_retry.WithCodes(retryCodes...))
	logger.Errorw(ctx, "Configuration values", log.Fields{"RETRY": a.config.MaxRetries, "TIMEOUT": a.config.RPCTimeout})
	//dummy2
