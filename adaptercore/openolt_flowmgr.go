...
func (f *OpenOltFlowMgr) addFlowToDevice(logicalFlow *ofp.OfpFlowStats, deviceFlow *openoltpb2.Flow) (bool, error) {
	...
	*/
	...
	if err := f.registerFlow(logicalFlow, deviceFlow); err != nil {
		log.Errorw("Failed to register flow", log.Fields{"error": err, "logicalFlow": logicalFlow, "deviceFlow": deviceFlow})
		return false, err
	}
...
func (f *OpenOltFlowMgr) clearFlowFromResourceManager(flow *ofp.OfpFlowStats, flowDirection string) error {
	...
	if err != nil {
		log.Errorw("Failed to extract infromation from flow", log.Fields{"error": err, "flow": flow})
		return err
	}
...
func (f *OpenOltFlowMgr) AddFlow(flow *ofp.OfpFlowStats, flowMetadata *voltha.FlowMetadata) error {
	...
	if ethType, ok := classifierInfo[EthType]; ok && ethType.(uint32) == LldpEthType {
		...
		if err := f.addLLDPFlow(flow, portNo); err != nil {
			log.Errorw("Failed to add LLDP Flow", log.Fields{"error": err})
			return err
		}
		return nil
	...
	if ipProto, ok := classifierInfo[IPProto]; ok && ipProto.(uint32) == IPProtoDhcp {
		if udpSrc, ok := classifierInfo[UDPSrc]; ok && (udpSrc.(uint32) == uint32(67) || udpSrc.(uint32) == uint32(546)) {
			...
			if err := f.addDHCPTrapFlowOnNNI(flow, classifierInfo, portNo); err != nil {
				log.Errorw("Failed to add dhcp trap flows on nni", log.Fields{"error": err, "flow": flow, "portno": portNo})
				return err
			}
			return nil
		}
	...
	if isIgmpTrapDownstreamFlow(classifierInfo) {
		...
		if err := f.addIgmpTrapFlowOnNNI(flow, classifierInfo, portNo); err != nil {
			log.Errorw("Failed to add igmp trap flows on nni", log.Fields{"error": err, "flow": flow, "portNo": portNo})
			return err
		}
		return nil
	}
