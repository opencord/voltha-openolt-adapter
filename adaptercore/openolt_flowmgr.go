	...
	if len(updatedFlows) >= 0 {
		...
		if len(updatedFlows) == 0 {
			...
			if !ok {
				...
				if err := f.sendDeleteTcontToChild(Intf, uint32(onuID), uint32(uniID), uint32(techprofileInst.UsScheduler.AllocID), tpPath); err != nil {log.Errorw("error-processing-delete-tcont-towards-onu",
