...
func (f *OpenOltFlowMgr) divideAndAddFlow(intfID uint32, onuID uint32, uniID uint32, portNo uint32, classifierInfo map[string]interface{}, actionInfo map[string]interface{}, flow *ofp.OfpFlowStats, TpId uint32, UsMeterId uint32, DsMeterId uint32) {
	var allocId []uint32
	var gemPorts []uint32
	var gemPort uint32
	...
	allocId, gemPorts, TpInst = f.createTcontGemports(intfID, onuID, uniID, uni, portNo, TpId, UsMeterId, DsMeterId)
	...
	if ipProto, ok := classifierInfo[IPProto]; ok {
		if ipProto.(uint32) == IPProtoDhcp {
			...
			if pcp, ok := classifierInfo[VlanPcp]; ok {
				...
				//Adding DHCP downstream flow
				//f.addDHCPTrapFlowOnNNI(intfID, onuId, uniId, portNo, classifierInfo, actionInfo, flow, allocId[0], gemPort)
			} else {
				...
				//Adding DHCP downstream flow to all gemports
				//installFlowOnAllGemports(f.addDHCPTrapFlowOnNNI, nil, args, classifierInfo, actionInfo, flow, gemPorts, DhcpFlow)
			}
...
func (f *OpenOltFlowMgr) CreateSchedulerQueues(Dir tp_pb.Direction, IntfId uint32, OnuId uint32, UniId uint32, UniPort uint32, TpInst *tp.TechProfile, MeterId uint32) error {

	log.Debugw("CreateSchedulerQueues", log.Fields{"Dir": Dir, "IntfId": IntfId, "OnuId": OnuId,
		"UniId": UniId, "MeterId": MeterId, "TpInst": *TpInst})
	...
	*/
	...
	KvStoreMeterId, err := f.resourceMgr.GetMeterIdForOnu(Direction, IntfId, OnuId, UniId)
	if err != nil {
		log.Error("Failed to get meter id for intf %d, onuid %d, uniid %d", IntfId, OnuId, UniId)
		return err
	}
	if KvStoreMeterId != 0 {
		if KvStoreMeterId == MeterId {
			log.Debug("Scheduler already created for upstream")
			return nil
		} else {
			log.Errorw("Dynamic meter update not supported", log.Fields{"KvStoreMeterId": KvStoreMeterId, "MeterId-in-flow": MeterId})
	...
	meterConfig, err := f.deviceHandler.GetMeterConfigFromCore(MeterId)
	if meterConfig == nil {
		log.Errorw("Failed to get meter config", log.Fields{"MeterId": MeterId})
		return errors.New("Failed to get meter config- timeout")
	} else if len(meterConfig.Bands) < MaxMeterBand {
		log.Warnw("Invalid number of bands in meter", log.Fields{"Bands": meterConfig.Bands})
	 ...
	 */
	if err := f.resourceMgr.UpdateMeterIdForOnu(Direction, IntfId, OnuId, UniId, MeterId); err != nil {
...
func (f *OpenOltFlowMgr) RemoveSchedulerQueues(Dir tp_pb.Direction, IntfId uint32, OnuId uint32, UniId uint32, UniPort uint32, TpInst *tp.TechProfile) error {
	...
	KVStoreMeterId, err := f.resourceMgr.GetMeterIdForOnu(Direction, IntfId, OnuId, UniId)
	if err != nil {
		log.Errorf("Failed to get Meter id for Onu %d", OnuId)
		return err
	}
	if KVStoreMeterId == 0 {
	...
	// FIXME: Meter config also could be stored in KV store so that no need to query back to core
	meterConfig, err := f.deviceHandler.GetMeterConfigFromCore(KVStoreMeterId)
	if meterConfig == nil {
		log.Errorw("Failed to get meter config", log.Fields{"MeterId": KVStoreMeterId})
		return err
	} else if len(meterConfig.Bands) < MaxMeterBand {
		log.Warnw("Invalid number of bands in meter", log.Fields{"Bands": meterConfig.Bands})
		return errors.New("Invalid number of bands in meter")
	...
	if err != nil {
		log.Errorf("Failed to remove meter id for onu %d, meter id %d", OnuId, KVStoreMeterId)
	}
	log.Debugw("Removed-meter-from-KV-store successfully", log.Fields{"MeterId": KVStoreMeterId, "dir": Direction})
	return err
}

// This function allocates tconts and GEM ports for an ONU, currently one TCONT is supported per ONU
func (f *OpenOltFlowMgr) createTcontGemports(intfID uint32, onuID uint32, uniID uint32, uni string, uniPort uint32, TpID uint32, UsMeterID uint32, DsMeterID uint32) ([]uint32, []uint32, *tp.TechProfile) {
	var allocID []uint32
	...
	if UsMeterID != 0 {
		if err := f.CreateSchedulerQueues(tp_pb.Direction_UPSTREAM, intfID, onuID, uniID, uniPort, tech_profile_instance, UsMeterID); err != nil {
			log.Errorw("CreateSchedulerQueues Failed-upstream", log.Fields{"error": err, "meterID": UsMeterID})
			return nil, nil, nil
		}
	}
	if DsMeterID != 0 {
		if err := f.CreateSchedulerQueues(tp_pb.Direction_DOWNSTREAM, intfID, onuID, uniID, uniPort, tech_profile_instance, DsMeterID); err != nil {
...
func (f *OpenOltFlowMgr) AddFlow(flow *ofp.OfpFlowStats) {
	...
	*/
	...
	f.divideAndAddFlow(intfId, onuId, uniId, portNo, classifierInfo, actionInfo, flow, TpID, UsMeterID, DsMeterID)
}
