...
func (f *OpenOltFlowMgr) generateStoredFlowID(flowID uint32, direction string) (uint64, error) {
	if direction == Upstream {
		...
		return 0, NewErrInvalidValue(ErrFields{"direction": direction}, nil)
	}
}
...
func (f *OpenOltFlowMgr) CreateSchedulerQueues(ctx context.Context, sq schedQueue) error {
	...
	if KvStoreMeter != nil {
		...
		return NewErrInvalidValue(ErrFields{"unsupported": "meter-id"}, nil)
	}
	...
	if meterConfig == nil {
		log.Errorw("Could-not-get-meterbands-from-flowMetadata", log.Fields{"flowMetadata": sq.flowMetadata,
			"MeterID": sq.meterID})
		return NewErrNotFound("meterbands", ErrFields{"flow-metadata": sq.flowMetadata}, nil)
...
func (f *OpenOltFlowMgr) populateTechProfilePerPonPort() error {
	...
	if tpCount != int(f.resourceMgr.DevInfo.GetPonPorts()) {
		...
		return NewErrInvalidValue(ErrFields{
			"numofTech":   tpCount,
			"numPonPorts": f.resourceMgr.DevInfo.GetPonPorts()}, nil)
	}
...
func (f *OpenOltFlowMgr) addDownstreamDataFlow(ctx context.Context, intfID uint32, onuID uint32, uniID uint32,
	...
	if ok {
		...
		return NewErrInvalidValue(ErrFields{"vlan-id": VlanVid}, nil)
	}
...
func (f *OpenOltFlowMgr) addHSIAFlow(ctx context.Context, intfID uint32, onuID uint32, uniID uint32, portNo uint32, classifier map[string]interface{},
	...
	*/
	...
	if err != nil {
		...
		return NewErrNotFound("hsia-flow-id", ErrFields{"direction": direction}, err)
	}
	classifierProto, err := makeOpenOltClassifierField(classifier)
	if err != nil {
		log.Error("Error in making classifier protobuf for hsia flow")
		return NewErrInvalidValue(ErrFields{"classifier": classifier}, err)
	}
	...
	if err != nil {
		...
		return NewErrInvalidValue(ErrFields{"action": action}, err)
	}
	...
	if err := f.addFlowToDevice(ctx, logicalFlow, &flow); err != nil {
		return NewErrFlowOp("add", flowID, nil, err)
	}
	...
	if err := f.updateFlowInfoToKVStore(ctx, flow.AccessIntfId,
		...
		return NewErrPersistence("update", "flow", flowID, ErrFields{"flow": flow}, err)
	}
	return nil
}
...
func (f *OpenOltFlowMgr) addDHCPTrapFlow(ctx context.Context, intfID uint32, onuID uint32, uniID uint32, portNo uint32, classifier map[string]interface{}, action map[string]interface{}, logicalFlow *ofp.OfpFlowStats, allocID uint32, gemPortID uint32) error {
	...
	if err != nil {
		log.Error("Failed to get nniIntf ID")
		return NewErrNotFound("nni-interface-id", ErrFields{
			"classifier": classifier,
			"action":     action},
			err)
	}
	...
	if err != nil {
		...
		return NewErrNotFound("flow", ErrFields{
			"interface-id": intfID,
			"gem-port":     gemPortID,
	...
	if err := f.addFlowToDevice(ctx, logicalFlow, &dhcpFlow); err != nil {
		...
		return NewErrFlowOp("add", flowID, ErrFields{"dhcp-flow": dhcpFlow}, err)
	}
	...
	if err := f.updateFlowInfoToKVStore(ctx, dhcpFlow.AccessIntfId,
		...
		return NewErrPersistence("update", "flow", dhcpFlow.FlowId, ErrFields{"flow": dhcpFlow}, err)
	}
...
func (f *OpenOltFlowMgr) addUpstreamTrapFlow(ctx context.Context, intfID uint32, onuID uint32, uniID uint32, portNo uint32, classifier map[string]interface{},
	...
	if err != nil {
		...
		return NewErrNotFound("flow-id", ErrFields{
			"interface-id": intfID,
			"oni-id":       onuID,
			"cookie":       flowStoreCookie,
			"flow-type":    flowType},
	...
	if err := f.addFlowToDevice(ctx, logicalFlow, &flow); err != nil {
		log.Errorw("failed-to-add-flow", log.Fields{"error": err})
		return NewErrFlowOp("add", flowID, ErrFields{"flow": flow}, err)
	}
	...
	if err := f.updateFlowInfoToKVStore(ctx, flow.AccessIntfId,
		...
		return NewErrPersistence("update", "flow", flow.FlowId, ErrFields{"flow": flow}, err)
	}
...
func (f *OpenOltFlowMgr) addEAPOLFlow(ctx context.Context, intfID uint32, onuID uint32, uniID uint32, portNo uint32, logicalFlow *ofp.OfpFlowStats, allocID uint32, gemPortID uint32, vlanID uint32, classifier map[string]interface{}, action map[string]interface{}) error {
	...
	if err != nil {
		log.Error("Error in making classifier protobuf for ul flow")
		return NewErrInvalidValue(ErrFields{"classifier": uplinkClassifier}, err)
	}
	log.Debugw("Created classifier proto", log.Fields{"classifier": *classifierProto})
	actionProto, err := makeOpenOltActionField(uplinkAction)
	if err != nil {
		log.Error("Error in making action protobuf for ul flow")
		return NewErrInvalidValue(ErrFields{"action": uplinkAction}, err)
	}
	log.Debugw("Created action proto", log.Fields{"action": *actionProto})
	networkIntfID, err := getNniIntfID(classifier, action)
	if err != nil {
	...
	if err := f.addFlowToDevice(ctx, logicalFlow, &upstreamFlow); err != nil {
		log.Errorw("failed-to-add-flow", log.Fields{"error": err})
		return NewErrFlowOp("add", uplinkFlowID, ErrFields{"flow": upstreamFlow}, err)
	}
	...
	if err := f.updateFlowInfoToKVStore(ctx, upstreamFlow.AccessIntfId,
		...
		return NewErrPersistence("update", "flow", upstreamFlow.FlowId, ErrFields{"flow": upstreamFlow}, err)
	}
...
func makeOpenOltClassifierField(classifierInfo map[string]interface{}) (*openoltpb2.Classifier, error) {
	...
	if pktTagType, ok := classifierInfo[PacketTagType].(string); ok {
		...
		default:
			...
			return nil, NewErrInvalidValue(ErrFields{"packet-tag-type": pktTagType}, nil)
		}
	}
...
func makeOpenOltActionField(actionInfo map[string]interface{}) (*openoltpb2.Action, error) {
	...
	if _, ok := actionInfo[PopVlan]; ok {
		...
		return nil, NewErrInvalidValue(ErrFields{"action-command": actionInfo}, nil)
	}
	return &action, nil
}
...
func (f *OpenOltFlowMgr) addLLDPFlow(ctx context.Context, flow *ofp.OfpFlowStats, portNo uint32) error {
	...
	if err != nil {
		log.Errorw("invalid-port-number",
			log.Fields{
				"port-number": portNo,
				"error":       err})
		return NewErrInvalidValue(ErrFields{"nni-port-number": portNo}, err)
	}
	...
	if err != nil {
		...
		return NewErrInvalidValue(ErrFields{"classifier": classifierInfo}, err)
	}
	log.Debugw("Created classifier proto", log.Fields{"classifier": *classifierProto})
	actionProto, err := makeOpenOltActionField(actionInfo)
	if err != nil {
		log.Error("Error in making action protobuf for LLDP trap on nni flow")
		return NewErrInvalidValue(ErrFields{"action": actionInfo}, err)
	}
	log.Debugw("Created action proto", log.Fields{"action": *actionProto})
	...
	if err := f.addFlowToDevice(ctx, flow, &downstreamflow); err != nil {
		log.Errorw("failed-to-add-flow", log.Fields{"error": err})
		return NewErrFlowOp("add", flowID, ErrFields{"flow": downstreamflow}, err)
	}
	...
	if err := f.updateFlowInfoToKVStore(ctx, int32(networkInterfaceID),
		...
		return NewErrPersistence("update", "flow", flowID, ErrFields{"flow": downstreamflow}, err)
	}
	return nil
}
...
func (f *OpenOltFlowMgr) getOnuChildDevice(intfID uint32, onuID uint32) (*voltha.Device, error) {
	...
	if onuDevice == nil {
		...
		return nil, NewErrNotFound("onu", ErrFields{
...
func (f *OpenOltFlowMgr) handleFlowWithGroup(ctx context.Context, actionInfo, classifierInfo map[string]interface{}, flow *ofp.OfpFlowStats) error {
	...
	if err != nil {
		...
		return NewErrNotFound("multicast-in-port", ErrFields{"classifier": classifierInfo}, err)
	}
	...
	if err != nil {
		log.Errorw("invalid-in-port-number",
			log.Fields{
				"port-number": inPort,
				"error":       err})
		return NewErrInvalidValue(ErrFields{"nni-in-port-number": inPort}, err)
	}
	...
	if err != nil {
		...
		return NewErrNotFound("multicast-flow-id", ErrFields{
			"interface-id": networkInterfaceID,
			"onu-id":       onuID,
			"uni-id":       uniID,
			"gem-port-id":  gemPortID,
	...
	if err = f.addFlowToDevice(ctx, flow, &multicastFlow); err != nil {
		log.Errorw("failed-to-add-flow", log.Fields{"error": err})
		return NewErrFlowOp("add", flowID, ErrFields{"flow": multicastFlow}, err)
	}
	...
	if err = f.updateFlowInfoToKVStore(ctx, int32(networkInterfaceID),
		...
		return NewErrPersistence("update", "flow", flowID, ErrFields{"flow": multicastFlow}, err)
	}
	return nil
}
...
func (f *OpenOltFlowMgr) getInPortOfMulticastFlow(ctx context.Context, classifierInfo map[string]interface{}) (uint32, error) {
	...
	return 0, NewErrNotFound("nni-port", nil, e)
}
...
func (f *OpenOltFlowMgr) getOnuIDfromGemPortMap(serialNumber string, intfID uint32, gemPortID uint32) (uint32, error) {
	...
	return uint32(0), NewErrNotFound("onu-id", ErrFields{
...
func (f *OpenOltFlowMgr) addDHCPTrapFlowOnNNI(ctx context.Context, logicalFlow *ofp.OfpFlowStats, classifier map[string]interface{}, portNo uint32) error {
	...
	*/
	...
	if err != nil {
		log.Error("Failed to get nniIntf ID")
		return NewErrNotFound("nni-intreface-id", ErrFields{
			"classifier": classifier,
			"action":     action},
			err)
	}
	...
	if err != nil {
		...
		return NewErrNotFound("dhcp-trap-nni-flow-id", ErrFields{
			"interface-id": networkInterfaceID,
			"onu-id":       onuID,
			"uni-id":       uniID,
			"gem-port-id":  gemPortID,
...
func (f *OpenOltFlowMgr) addIgmpTrapFlowOnNNI(ctx context.Context, logicalFlow *ofp.OfpFlowStats, classifier map[string]interface{}, portNo uint32) error {
	...
	*/
	...
	if err != nil {
		...
		return NewErrNotFound("igmp-flow-id", ErrFields{
			"interface-id": networkInterfaceID,
			"onu-id":       onuID,
			"uni-id":       uniID,
			"gem-port-id":  gemPortID,
...
func verifyMeterIDAndGetDirection(MeterID uint32, Dir tp_pb.Direction) (string, error) {
	if MeterID == 0 { // This should never happen
		...
		return "", NewErrInvalidValue(ErrFields{"meter-id": MeterID}, nil)
	}
	...
	for _, action := range flows.GetActions(flow) {
		if action.Type == flows.OUTPUT {
			if out := action.GetOutput(); out != nil {
				...
				log.Error("Invalid output port in action")
				return NewErrInvalidValue(ErrFields{"output-port": nil}, nil)
			}
			...
			if out := action.GetSetField(); out != nil {
				if field := out.GetField(); field != nil {
					if ofClass := field.GetOxmClass(); ofClass != ofp.OfpOxmClass_OFPXMC_OPENFLOW_BASIC {
						...
						return NewErrInvalidValue(ErrFields{"openflow-class": ofClass}, nil)
					}
			...
			return NewErrInvalidValue(ErrFields{"action-type": action.Type}, nil)
		}
	}
	return nil
}
	...
	if isControllerFlow := IsControllerBoundFlow(actionInfo[Output].(uint32)); isControllerFlow {
		...
		if portType := IntfIDToPortTypeName(classifierInfo[InPort].(uint32)); portType == voltha.Port_PON_OLT {
			if uniPort := flows.GetChildPortFromTunnelId(flow); uniPort != 0 {
				...
				return NewErrNotFound("child-in-port", ErrFields{"flow": flow}, nil)
			}
		}
	} else {
		...
		if portType := IntfIDToPortTypeName(actionInfo[Output].(uint32)); portType == voltha.Port_PON_OLT {
			if uniPort := flows.GetChildPortFromTunnelId(flow); uniPort != 0 {
				...
				return NewErrNotFound("out-port", ErrFields{"flow": flow}, nil)
			}
			...
			if uniPort := flows.GetChildPortFromTunnelId(flow); uniPort != 0 {
				...
				log.Debug("upstream-pon-to-nni-port-flow, no-inport-in-tunnelid", log.Fields{"InPort": classifierInfo[InPort].(uint32),
					"outPort": actionInfo[Output].(uint32)})
				return NewErrNotFound("nni-port", ErrFields{"flow": flow}, nil)
			}
		}
	}
	return nil
...
func getTpIDFromFlow(flow *ofp.OfpFlowStats) (uint32, error) {
	...
	*/
	...
	if metadata == 0 {
		...
		return 0, NewErrNotFound("metadata", ErrFields{"flow": flow}, nil)
	}
...
func (f *OpenOltFlowMgr) GetFlowGroupFromKVStore(ctx context.Context, groupID uint32, cached bool) (*ofp.OfpGroupEntry, bool, error) {
	...
	if err != nil {
		...
		return nil, false, NewErrNotFound("flow-group", ErrFields{"group-id": groupID}, err)
	}
	if exists {
