			...
			if f.isGemPortUsedByAnotherFlow(gemPK) {
				...
				for i, flowIDinMap := range flowIDs {
					if flowIDinMap == flowID {
						...
						if err := f.resourceMgr.UpdateFlowIDsForGem(Intf, uint32(gemPortID), flowIDs); err != nil {
							log.Errorw("Failed to update flow ids for gem", log.Fields{"error": err, "gemportID": gemPortID, "flowIDs": flowIDs})
							return err
						}
						break
					}
