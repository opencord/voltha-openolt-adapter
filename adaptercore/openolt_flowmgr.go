...
type OpenOltFlowMgr struct {
	...
	gemPortUsedByFlows map[gemPortKey][]uint32 //gem port id to flow ids
}
...
func NewFlowManager(dh *DeviceHandler, rsrcMgr *rsrcMgr.OpenOltResourceMgr) *OpenOltFlowMgr {
	...
	flowMgr.gemPortUsedByFlows = make(map[gemPortKey][]uint32)
...
func (f *OpenOltFlowMgr) registerFlow(flowFromCore *ofp.OfpFlowStats, deviceFlow *openoltpb2.Flow) {
	...
	gemPortKey := gemPortKey{uint32(deviceFlow.AccessIntfId), uint32(deviceFlow.GemportId)}
	flowIDList, ok := f.gemPortUsedByFlows[gemPortKey]
	if !ok {
		flowIDList = []uint32{deviceFlow.FlowId}
	}
	flowIDList = append(flowIDList, deviceFlow.FlowId)
	f.gemPortUsedByFlows[gemPortKey] = flowIDList
...
func (f *OpenOltFlowMgr) clearFlowFromResourceManager(flow *ofp.OfpFlowStats, flowID uint32, flowDirection string) {
	...
	if len(updatedFlows) >= 0 {
		...
		if len(updatedFlows) == 0 {
			...
			ok, value := f.isTechProfileUsedByAnotherGem(ponIntf, onuID, uniID, techprofileInst, uint32(gemPortID))
			if ok {
				log.Debugf("Tech profile %d is used by another gem %d", tpID, value)
				gemPortKey := gemPortKey{ponIntf, uint32(gemPortID)}
				if !f.isGemPortUsedByAnotherFlow(gemPortKey) {
					...
					delete(f.gemPortUsedByFlows, gemPortKey)
					return
				}
				flowIDs := f.gemPortUsedByFlows[gemPortKey]
				for i, flowID := range flowIDs {
					if flowID == uint32(flow.Id) {
						copy(flowIDs[i:], flowIDs[i+1:])
						break
					}
				}
			...
			f.resourceMgr.RemoveAllocIDForOnu(ponIntf, onuID, uniID, techprofileInst.UsScheduler.AllocID)
			f.resourceMgr.RemoveGemPortIDForOnu(ponIntf, onuID, uniID, uint32(gemPortID))
			f.resourceMgr.RemoveGEMportPonportToOnuMapOnKVStore(uint32(gemPortID), ponIntf)
		}
	}
...
func (f *OpenOltFlowMgr) RemoveFlow(flow *ofp.OfpFlowStats) {
	...
	for index, curFlow := range deviceFlowsToRemove {
		...
		if ok := f.removeFlowFromDevice(&removeFlowMessage); ok {
			...
			go f.clearFlowFromResourceManager(flow, uint32(id), direction) //TODO: Take care of the limitations
		}
...
func (f *OpenOltFlowMgr) isGemPortUsedByAnotherFlow(gemPortKey gemPortKey) bool {
	flowIDList := f.gemPortUsedByFlows[gemPortKey]
	if len(flowIDList) > 1 {
		return true
	}
	return false
