...
func makeOpenOltClassifierField(classifierInfo map[string]interface{}) *openoltpb2.Classifier {
	...
	setReservedVlanClassifier(classifierInfo, &classifier)
...
func setReservedVlanClassifier(classifierInfo map[string]interface{}, classifier *openoltpb2.Classifier) {
...
func (f *OpenOltFlowMgr) clearFlowFromResourceManager(flow *ofp.OfpFlowStats, flowID uint32, flowDirection string) {
	...
	if ethType == LldpEthType {
		...
		f.resourceMgr.FreeFlowID(networkInterfaceID, uint32(onuID), uint32(uniID), flowID)
		return
	}
