...
import (
	...
	tpconfig "github.com/opencord/voltha-go/adapters/common/techprofile/config"
	fd "github.com/opencord/voltha-go/rw_core/flow_decomposition"
	"github.com/opencord/voltha-go/common/log"
	ofp "github.com/opencord/voltha-go/protos/openflow_13"
	openolt_pb2 "github.com/opencord/voltha-go/protos/openolt"
	voltha "github.com/opencord/voltha-go/protos/voltha"
)
...
func NewFlowManager(dh *DeviceHandler,rsrcMgr *OpenOltResourceMgr )*OpenOltFlowMgr{
...
func( f *OpenOltFlowMgr) createTcontGemports(intfId uint32, onuId uint32, uniId uint32,uni string,uniPort uint32, tableID uint32)([]uint32,[]uint32){
	...
	if tableID < tpconfig.DEFAULT_TECH_PROFILE_TABLE_ID{
		tableID = tpconfig.DEFAULT_TECH_PROFILE_TABLE_ID
	}
...
func(f *OpenOltFlowMgr) storeTcontsGEMPortsIntoKVStore(intfId uint32, onuId uint32, uniId uint32,allocID []uint32, gemPortIDs []uint32){
	...
	/* Update the allocated alloc_id and gem_port_id for the ONU/UNI to KV store  */
        if err := f.resourceMgr.UpdateAllocIdsForOnu(intfId, onuId, allocID); err != nil{
...
func (f *OpenOltFlowMgr) addEAPOLFlow(intfId uint32, onuId uint32, uniId uint32, portNo uint32, logicalFlow *ofp.OfpFlowStats, allocId uint32, gemPortId uint32, vlanId uint32 ){
		...
		upstreamFlow = openolt_pb2.Flow{AccessIntfId: int32(intfId) ,
			...
			PortNo: portNo}
                // TODO: Core already upto date on this flow  , remove it 
		upstreamLogicalFlow := *logicalFlow
                matchFields := []ofp.OfpOxmField {ofp.OfpOxmField{Field: &ofp.OfpOxmField_OfbField{OfbField: fd.VlanVid(uint32(vlanId | 0x1000))}}}
                fields := fd.MkOxmFields(matchFields)
                for _,field:= range(fields){
		    upstreamLogicalFlow.Match.OxmFields = append(upstreamLogicalFlow.Match.OxmFields,field)
                }
		upstreamLogicalFlow.Match.Type = ofp.OfpMatchType_OFPMT_OXM
		if ok := f.addFlowToDevice(&upstreamFlow, &upstreamLogicalFlow); ok{
		...
		if vlanId == DEFAULT_MGMT_VLAN{
			...
			*/
			...
			downstreamFlow = openolt_pb2.Flow{AccessIntfId: int32(intfId) ,
				...
				Cookie: logicalFlow.Cookie,
				PortNo: portNo}
			// Create downstream logical flow 
			downstreamLogicalFlow := ofp.OfpFlowStats{
				Id: logicalFlow.Id,
				Cookie: logicalFlow.Cookie,
				TableId: logicalFlow.TableId,
				Priority: logicalFlow.Priority,
				Flags: logicalFlow.Flags,
                                Match: &ofp.OfpMatch{},
                                Instructions:[]*ofp.OfpInstruction {&ofp.OfpInstruction{}}}
			// Fill match field
                        // TODO : Already core is up to data , no need to pass logical flow remove it
                        matchFields := []ofp.OfpOxmField {ofp.OfpOxmField{Field: &ofp.OfpOxmField_OfbField{OfbField: fd.VlanVid(uint32(specialVlanDlFlow | 0x1000))}},
                                                           ofp.OfpOxmField{Field: &ofp.OfpOxmField_OfbField{OfbField:fd.InPort(fd.GetOutPort(logicalFlow))}}}
                        fields := fd.MkOxmFields(matchFields)
                        for _,field:= range(fields){
		            downstreamLogicalFlow.Match.OxmFields = append(downstreamLogicalFlow.Match.OxmFields,field)
                        }
		        downstreamLogicalFlow.Match.Type = ofp.OfpMatchType_OFPMT_OXM
			// Fill action instruction fields
			actionsFields := []*ofp.OfpAction{fd.Output(MkUniPortNum(intfId, onuId, uniId))}
                        actions := fd.MkInstructionsFromActions(actionsFields)
                        for _,action := range(actions){
			    downstreamLogicalFlow.Instructions = append(downstreamLogicalFlow.Instructions,action)
                        }
			if ok := f.addFlowToDevice(&downstreamFlow, &downstreamLogicalFlow); ok{
...
func(f *OpenOltFlowMgr) getTPpath(intfId uint32, uni string)string{
	...
	*/
	return f.techprofile[intfId].GetTechProfileInstanceKVPath(tpconfig.DEFAULT_TECH_PROFILE_TABLE_ID, uni)
}
...
func (f *OpenOltFlowMgr) addFlowToDevice(deviceFlow *openolt_pb2.Flow, logicalFlow *ofp.OfpFlowStats)bool{
    ...
    if err != nil{
         log.Errorw("Failed to Add flow to device", log.Fields{"err": err, "logicalflow": *logicalFlow, "deviceFlow": deviceFlow})
         return false
    }
    log.Debugw("Flow added to device successfuly ",log.Fields{"flow": *deviceFlow,"logicalflow": *logicalFlow })
    return true
}
...
func addLLDPFlow(flow *ofp.OfpFlowStats, portNo uint32){
	/* TODO : LLDP will be implemented later */
}
...
/*func(f *OpenOltFlowMgr) getDevicePorts(device *voltha.Device, portType voltha.Port_PortType)*voltha.Ports{
	ports, err := f.deviceHandler.CoreProxy.GetPorts(nil,device.Id, f.deviceHandler.DeviceId, portType)
        if err != nil{
		log.Errorw("could-not-find-device-ports",log.Fields{"device":device.Id,"parent_Device": f.deviceHandler.DeviceId })
		return nil
	}
	return ports
}

func(f *OpenOltFlowMgr) getLogicalPort(portLabel string)*voltha.LogicalPort{
	lport, err := f.deviceHandler.CoreProxy.GetLogicalPort(nil,f.deviceHandler.DeviceId,portLabel)
        if err != nil{
		log.Errorw("could-not-find-logical-device-ports",log.Fields{"olt_Device": f.deviceHandler.DeviceId  })
		return nil
	}
	return lport
}*/
...
func findNextFlow(flow *ofp.OfpFlowStats)*ofp.OfpFlowStats{
	/* TODO : EAPOL uses default vlan , does not have meta data , so handle later */
       log.Info("Unimplemented")
       return nil
}
...
func (f *OpenOltFlowMgr) clear_flows_and_scheduler_for_logical_port(childDevice *voltha.Device,logicalPort  *voltha.LogicalPort){
	/* TODO: implement later during delete logical port  */
       log.Info("Unimplemented")
       return
}
	...
	for _,field := range fd.GetOfbFields(flow){
		if field.Type == fd.ETH_TYPE {
			...
			classifierInfo[IN_PORT] = field.GetPort()
                        ...
                        /* TODO : Currently onos does not give proper inport as uniport , so hardcode to unblock testing */
			classifierInfo[IN_PORT] = uint32(2064)
