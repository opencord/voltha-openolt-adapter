...
func (f *OpenOltFlowMgr) sendDeleteGemPortToChild(intfID uint32, onuID uint32, uniID uint32, gemPortId uint32) error {
	...
	delGemPortMsg := &ic.InterAdapterDeleteGemPortMessage{UniId: uniID, GemPortId: gemPortId}
...
func (f *OpenOltFlowMgr) sendDeleteTcontToChild(intfID uint32, onuID uint32, uniID uint32, allocId uint32) error {
	onuDevice, err := f.getOnuChildDevice(intfID, onuID)
	if err != nil {
		log.Errorw("error-fetching-child-device-from-core", log.Fields{"onuId": onuID})
		return err
	...
	delTcontMsg := &ic.InterAdapterDeleteTcontMessage{UniId: uniID, AllocId: allocId}
...
func (f *OpenOltFlowMgr) clearResources(flow *ofp.OfpFlowStats, Intf uint32, onuID int32, uniID int32,
	...
	if len(updatedFlows) >= 0 {
		...
		if len(updatedFlows) == 0 {
			...
			go func() {
				if err := f.sendDeleteGemPortToChild(Intf, uint32(onuID), uint32(uniID), uint32(gemPortID)); err != nil {
			...
			if !ok {
				...
				go func() {
					if err := f.sendDeleteTcontToChild(Intf, uint32(onuID), uint32(uniID), uint32(techprofileInst.UsScheduler.AllocID)); err != nil {
