...
func (f *OpenOltFlowMgr) AddFlow(flow *ofp.OfpFlowStats, flowMetadata *voltha.FlowMetadata) error {
	...
	if flows.HasGroup(flow) {
		...
		if err := f.handleFlowWithGroup(actionInfo, classifierInfo, flow); err != nil {
			log.Errorw("Failed to handle flow with group", log.Fields{"error": err})
		}
		return nil
	}
	...
	if flows.HasGroup(flow) {
		// handle multicast flow
		if errFlowGrp := f.handleFlowWithGroup(actionInfo, classifierInfo, flow); err != nil {
