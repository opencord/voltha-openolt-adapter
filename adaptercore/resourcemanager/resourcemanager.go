...
func (RsrcMgr *OpenOltResourceMgr) AddMcastQueueForIntf(intf uint32, gem uint32, servicePriority uint32) error {
	...
	if err != nil {
		log.Error("Failed to get multicast queue info for interface", log.Fields{"error": err, "intf": intf})
		return err
	}
	if mcastQueues == nil {
	...
	val, err = json.Marshal(mcastQueues)
	if err != nil {
		log.Error("Failed to marshal data", log.Fields{"error": err})
		return err
	}
	...
	log.Debugw("added flowid list for gem to kv successfully", log.Fields{"path": path, "flowidlist": mcastQueues[intf]})
	return nil
}
