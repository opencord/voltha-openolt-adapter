	...
	if addr = net.ParseIP(host); addr == nil {
		...
		if ips, err = net.LookupHost(host); err == nil {
			...
			if addr = net.ParseIP(ips[0]); addr == nil {
				return "", LogAndReturn(log.DebugLevel, NewErrInvalidValue(log.Fields{"ip": ips[0]}, nil))
			}
			genmac = macifyIP(addr)
			log.Debugw("using-ip-as-mac", log.Fields{"host": ips[0], "mac": genmac})
			return genmac, nil
		}
		return "", LogAndReturn(log.DebugLevel,
			NewErrAdapter("cannot-resolve-hostname-to-ip", nil, err))
	}
...
func GetportLabel(portNum uint32, portType voltha.Port_PortType) (string, error) {
	...
	return "", LogAndReturn(log.DebugLevel,
		NewErrInvalidValue(log.Fields{"port-type": portType}, nil))
}
...
func (dh *DeviceHandler) addPort(intfID uint32, portType voltha.Port_PortType, state string) error {
	...
	if err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrNotFound("port-label", log.Fields{"port-number": portNum, "port-type": portType}, nil))
	}

	device, err := dh.coreProxy.GetDevice(context.TODO(), dh.device.Id, dh.device.Id)
	if err != nil || device == nil {
		return LogAndReturn(log.DebugLevel,
			NewErrNotFound("device", log.Fields{"device-id": dh.device.Id}, err))
	}
	if device.Ports != nil {
		for _, dPort := range device.Ports {
			if dPort.Type == portType && dPort.PortNo == portNum {
				...
				if err := dh.coreProxy.PortStateUpdate(context.TODO(), dh.device.Id, portType, portNum, operStatus); err != nil {
					return LogAndReturn(log.DebugLevel,
						NewErrAdapter("failed-to-update-port-state", log.Fields{
	...
	if err := dh.coreProxy.PortCreated(context.TODO(), dh.device.Id, port); err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrAdapter("Error-creating-port", log.Fields{
				"device-id": dh.device.Id,
...
func (dh *DeviceHandler) readIndications(ctx context.Context) error {
	...
	if err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrCommunication("fail-to-read-indications", log.Fields{"device-id": dh.device.Id}, err))
	}
	if indications == nil {
		return LogAndReturn(log.DebugLevel,
			NewErrInvalidValue(log.Fields{"indications": nil, "device-id": dh.device.Id}, nil))
	}
	/* get device state */
	...
	for {
		...
		if err == io.EOF {
			...
			if err != nil {
				return LogAndReturn(log.DebugLevel,
					NewErrCommunication("indication-read-failure", log.Fields{"device-id": dh.device.Id}, err))
			}
			continue
		}
		if err != nil {
...
func (dh *DeviceHandler) handleOltIndication(ctx context.Context, oltIndication *oop.OltIndication) error {
	...
	// Send or clear Alarm
	return LogAndReturn(log.DebugLevel, dh.eventMgr.oltUpDownIndication(oltIndication, dh.deviceID, raisedTs))
}
	...
	if dh.adminState == "down" {
		...
		go func() {
			if err := dh.readIndications(ctx); err != nil {
				LogAndReturn(log.ErrorLevel, err)
			}
		}()
		return nil
	}
...
func (dh *DeviceHandler) omciIndication(omciInd *oop.OmciIndication) error {
	...
	if err := dh.AdapterProxy.SendInterAdapterMessage(context.Background(), omciMsg,
		...
		return LogAndReturn(log.DebugLevel,
			NewErrCommunication("omci-request", log.Fields{
...
func (dh *DeviceHandler) onuDiscIndication(ctx context.Context, onuDiscInd *oop.OnuDiscIndication, sn string) error {
	...
	if sn != "" {
		...
		return LogAndReturn(log.DebugLevel,
			NewErrInvalidValue(log.Fields{"serial-number": sn}, nil))
	}
	...
	if err != nil {
		...
		if e, ok := status.FromError(err); ok {
			...
			case codes.DeadlineExceeded:
				...
				return LogAndReturn(log.DebugLevel,
					NewErrTimeout("get-child-device", log.Fields{"device-id": dh.device.Id}, err))
			}
		}
	}
	...
	if onuDevice == nil {
		...
		if err != nil {
			...
			dh.discOnus.Delete(sn)
			return LogAndReturn(log.DebugLevel,
				NewErrAdapter("resource-manage-get-onu-id-failed", log.Fields{
		...
		if onuDevice, err = dh.coreProxy.ChildDeviceDetected(context.TODO(), dh.device.Id, int(parentPortNo),
			...
			return LogAndReturn(log.DebugLevel,
				NewErrAdapter("core-proxy-child-device-detected-failed", log.Fields{
					"pon-interface-id": ponintfid,
					"serial-number":    sn}, err))
		}
	...
	if err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrAdapter("failed-to-update-device-state", log.Fields{
...
func (dh *DeviceHandler) onuIndication(onuInd *oop.OnuIndication) error {
	...
	if err != nil || onuDevice == nil {
		return LogAndReturn(log.DebugLevel, NewErrNotFound("onu-device", errFields, err))
	}
...
func (dh *DeviceHandler) handlePacketIndication(ctx context.Context, packetIn *oop.PacketIndication) error {
	...
	if err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrNotFound("logical-port", log.Fields{"packet": hex.EncodeToString(packetIn.Pkt)}, err))
	}
	...
	if err := dh.coreProxy.SendPacketIn(context.TODO(), dh.device.Id, logicalPortNum, packetIn.Pkt); err != nil {
		return LogAndReturn(log.DebugLevel,
			NewErrCommunication("send-packet-in", log.Fields{
				"destination": "core",
				"source":      dh.deviceType,
