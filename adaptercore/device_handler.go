...
func (dh *DeviceHandler) doStateConnected() error {
	...
	for _, port := range cloned.Ports {
		if port.AdminState == common.AdminState_DISABLED {
			if err := dh.invokeDisableorEnableIf(port, false); err != nil {
				log.Errorw("Error-occured-while-disbaling-port", log.Fields{"DeviceId": dh.deviceID, "port": port, "error": err})
				return err
			}
		} else {
...
// EnableNetworkIf to enable Pon interface
func (dh *DeviceHandler) EnableNetworkIf(port *voltha.Port) error {
	log.Debugw("enable-network-interface", log.Fields{"Device": dh.device, "port": port})
	return dh.invokeDisableorEnableIf(port, true)
}

// DisableNetworkIf to disable pon interface
func (dh *DeviceHandler) DisableNetworkIf(port *voltha.Port) error {
	log.Debugw("disable-network-interface", log.Fields{"Device": dh.device, "port": port})
	return dh.invokeDisableorEnableIf(port, false)
}

func (dh *DeviceHandler) invokeDisableorEnableIf(port *voltha.Port, enabler bool) error {
	log.Infow("invokeDisableorEnableIf", log.Fields{"port": port, "Enable ?": enabler})
	...
	if enabler {
		...
		if err != nil {
			log.Errorw("Error-while-enable-Pon-interface", log.Fields{"DeviceID": dh.device, "Port": port, "error": err})
			return err
		}
		// updating interface local cache for collecting stats
		dh.intfStatus[ponId] = true
		log.Infow("Enabled network interface", log.Fields{"out": out, "DeviceID": dh.device, "Port": port})
	} else {
		...
		log.Infow("disabled-network-interface", log.Fields{"out": out, "DeviceID": dh.device, "Port": port})
	}
