...
func (dh *DeviceHandler) onuDiscIndication(onuDiscInd *oop.OnuDiscIndication, sn string) {
	...
	if err != nil {
		...
		if e, ok := status.FromError(err); ok {
			...
			default:
				// the operation failed or timed out, this is a warning as the device will send a new
				// onuDiscoveryIndication
				log.Warnw("Error while reading child devices",
					log.Fields{"parent_id": dh.device.Id, "ponPort": onuDiscInd.GetIntfId(),
						"onuID": onuID, "sn": sn, "error": err})
				dh.discOnus.Delete(sn)
				return
			...
			log.Errorw("not able to parse error returned", log.Fields{"onuID": onuID, "sn": sn, "err": err})
		}
