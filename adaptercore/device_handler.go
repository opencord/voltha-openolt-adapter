	...
	if addr = net.ParseIP(host); addr == nil {
		...
		if ips, err = net.LookupHost(host); err == nil {
			...
			if addr = net.ParseIP(ips[0]); addr == nil {
				...
				return "", NewErrInvalidValue(ErrFields{"ip": ips[0]}, nil)
			}
...
func (dh *DeviceHandler) doStateDown(ctx context.Context) error {
	...
	if err != nil || device == nil {
		...
		return NewErrNotFound("device", ErrFields{"device-id": dh.device.Id}, err)
	}
...
func (dh *DeviceHandler) populateDeviceInfo() (*oop.DeviceInfo, error) {
	...
	if deviceInfo == nil {
		...
		return nil, NewErrInvalidValue(ErrFields{"device": nil}, nil)
	}
