...
func (oo *OpenOLT) Adopt_device(device *voltha.Device) error {
	...
	if device == nil {
		...
		return NewErrInvalidValue(log.Fields{"device": nil}, nil)
	}
...
func (oo *OpenOLT) Get_ofp_device_info(device *voltha.Device) (*ic.SwitchCapability, error) {
	...
	return nil, NewErrNotFound("device-handler", log.Fields{"device-id": device.Id}, nil)
}
...
func (oo *OpenOLT) Process_inter_adapter_message(msg *ic.InterAdapterMessage) error {
	...
	return NewErrNotFound("device-handler", log.Fields{"device-id": targetDevice}, nil)
}
...
func (oo *OpenOLT) Disable_device(device *voltha.Device) error {
	...
	log.Errorw("device-handler-not-set", log.Fields{"deviceId": device.Id})
	return NewErrNotFound("device-handler", log.Fields{"device-id": device.Id}, nil)
}
...
func (oo *OpenOLT) Receive_packet_out(deviceID string, egressPortNo int, packet *openflow_13.OfpPacketOut) error {
	...
	return NewErrNotFound("device-handler", log.Fields{"device-id": deviceID}, nil)
}
...
func (oo *OpenOLT) enableDisablePort(deviceID string, port *voltha.Port, enablePort bool) error {
	...
	if port == nil {
		...
		return NewErrInvalidValue(log.Fields{"port": nil}, nil)
	}
	if handler := oo.getDeviceHandler(deviceID); handler != nil {
