...
func NewEtcdCustomClient(ctx context.Context, addr string, timeout time.Duration, level log.LogLevel) (*EtcdClient, error) {
	...
	var err error
	pool, err := NewRoundRobinEtcdClientAllocator([]string{addr}, timeout, MaxPoolCapacity, MaxPoolUsage)
	if err != nil {
	...
	return &EtcdClient{pool: pool, keyReservations: reservations, lockToMutexMap: lockMutexMap,
...
startLoop:
	for {
		...
		// _ctx, _cancel := context.WithTimeout(context.Background(), 30*time.Second)
		if ok {
			_, err = client.Put(ctx, key, val, v3Client.WithLease(*leaseID))
...
func (c *EtcdClient) Reserve(ctx context.Context, key string, value interface{}, ttl time.Duration) (interface{}, error) {
	...
	resp, err := client.Grant(ctx, int64(ttl.Seconds()))
	if err != nil {
		logger.Error(ctx, err)
		return nil, err
	}
...
func (c *EtcdClient) Watch(ctx context.Context, key string, withPrefix bool) chan *Event {
	...
	// TODO:  Find a better way to release
	//defer c.pool.Put(client)
...
func (c *EtcdClient) AcquireLock(ctx context.Context, lockName string, timeout time.Duration) error {
	...
	defer c.pool.Put(client)
	session, _ := v3Concurrency.NewSession(client, v3Concurrency.WithContext(ctx))
