...
type Backend struct {
	...
	watches                 map[string]chan *kvstore.Event
	watchesClient           map[string]kvstore.Client
...
func NewBackend(ctx context.Context, storeType string, address string, timeout time.Duration, pathPrefix string) *Backend {
	...
	b := &Backend{
		...
		watches:                 make(map[string]chan *kvstore.Event),
		watchesClient:           make(map[string]kvstore.Client),
	}

	var err error
	b.Client, err = kvstore.NewEtcdClient(ctx, address, timeout, log.WarnLevel)
	if err != nil {
		logger.Errorw(ctx, "failed-to-create-kvstore-client", log.Fields{
...
func (b *Backend) List(ctx context.Context, key string) (map[string]*kvstore.KVPair, error) {
	...
	logger.Infow(ctx, "list-key", log.Fields{"key": key, "path": formattedPath, "time": time.Since(start), "error": err})
...
func (b *Backend) Get(ctx context.Context, key string) (*kvstore.KVPair, error) {
	...
	logger.Infow(ctx, "get-key", log.Fields{"key": key, "path": formattedPath, "time": time.Since(start), "error": err})
...
func (b *Backend) Put(ctx context.Context, key string, value interface{}) error {
	...
	logger.Infow(ctx, "put-key", log.Fields{"key": key, "path": formattedPath, "time": time.Since(start), "error": err})
...
func (b *Backend) Delete(ctx context.Context, key string) error {
	...
	logger.Infow(ctx, "delete-key", log.Fields{"key": key, "path": formattedPath, "time": time.Since(start), "error": err})
...
func (b *Backend) DeleteWithPrefix(ctx context.Context, prefixKey string) error {
	...
	logger.Infow(ctx, "delete-prefix-key", log.Fields{"key": prefixKey, "path": formattedPath, "time": time.Since(start), "error": err})
...
func (b *Backend) CreateWatch(ctx context.Context, key string, withPrefix bool) chan *kvstore.Event {
	...
	//b.watchesMutex.Lock()
	//defer b.watchesMutex.Unlock()
	//if ch, ok := b.watches[key]; ok {
	//	return ch
	//}
	//
	//ch := client.Watch(ctx, formattedPath, withPrefix)
	//b.watches[key] = ch
	//b.watchesClient[key] = client
	//logger.Infow(ctx, "creating-key-watch", log.Fields{"key": key, "path": formattedPath})
	//return ch
}
...
func (b *Backend) DeleteWatch(ctx context.Context, key string, ch chan *kvstore.Event) {
	...
	//	return
	//}
	//client, ok := b.watchesClient[key]
	//if !ok {
	//	return
	//}
	//
	//client.CloseWatch(ctx, formattedPath, ch)
	b.Client.CloseWatch(ctx, formattedPath, ch)
	//b.pool.Put(client)
	logger.Infow(ctx, "returning-watch-to-pool", log.Fields{"key": key, "path": formattedPath})
}
