...
class OpenoltDevice(object):
    ...
    def onu_indication(self, onu_indication):
        ...
        if onu_indication.oper_state == 'down':
            ...
            self.log.debug('onu-omci-event', onu_event=onu_indication)
    ...
    def omci_indication(self, omci_indication):
        ...
        self.log.debug('onu-omci-msg', onu_msg=omci_msg)
    ...
    def process_inter_adapter_message(self, request):
        ...
        try:
            if request.header.type == InterAdapterMessageType.OMCI_REQUEST:
                ...
                self.log.debug("inter-adapter-onu-omci-msg", omci_msg=omci_msg)
                ...
                self.log.debug("send-omci-message", onu_device=onu_device, omci_msg=omci_msg)

            else:
                self.log.error("unhandled-onu-oper-omci-request", request=request)
