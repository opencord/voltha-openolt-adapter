...
class OpenOltFlowMgr(object):
    ...
    def add_flow(self, flow):
        ...
        # controller bound trap flows.  figure out real port from metadata and set as in port
        if self.platform.is_controller(action_info[OUTPUT]):
            if self.platform.intf_id_to_port_type_name(classifier_info[IN_PORT]) == Port.PON_OLT:
                if fd.get_metadata_64_bit(flow) is not None:
                    ...
                    self.log.debug('pon-to-controller-flow-port-in-metadata', new_in_port=classifier_info[IN_PORT])
                else:
                    self.log.debug('pon-to-controller-flow-NO-PORT-in-metadata', in_port=classifier_info[IN_PORT],
