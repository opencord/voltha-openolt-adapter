...
func (f *OpenOltFlowMgr) GetUniPortByPonPortGemPort(ctx context.Context, intfID uint32, gemPortID uint32) (uint32, uint32, error) {
	...
	if ok {
		accurate := true
		if len(uniPortInfo) < 2 {
			//returning array should contain onuID and uniID
			accurate = false
		}
		if accurate {
			//return uni port from the cache
	...
	//If uni port is not found in cache try to get it from kv store, if found in kv store, update the cache and return.
