...
func (oo *OpenOLT) Get_ext_value(deviceID string, device *voltha.Device, valueparam voltha.ValueType_Type) (*voltha.ReturnValues, error) {
	...
	log.Infow("Get_values", log.Fields{"deviceId": deviceID, "onuid": device.Id})
	if handler := oo.getDeviceHandler(deviceID); handler != nil {
		log.Debugw("get-value", log.Fields{"deviceId": deviceID, "onu-id": device.Id})
		if resp, err = handler.getExtValue(device, valueparam); err != nil {
			log.Errorw("error-occurred-during-get-ext-value", log.Fields{"deviceID": deviceID, "onu-id": device.Id,
				"error": err})
			return nil, err
		}
	}
