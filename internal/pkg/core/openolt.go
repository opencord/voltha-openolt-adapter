...
func (oo *OpenOLT) Get_onu_distance(deviceID string, device *voltha.Device,onuid *voltha.ID, ) (*voltha.OnuDistance,error) {
	var err error
	distance := new(voltha.OnuDistance)
        ...
        if handler := oo.getDeviceHandler(deviceID); handler != nil {
                log.Debugw("get-onu-distance", log.Fields{"deviceId": deviceID, "onu-id": onuid.Id})
                if  distance, err = handler.getOnuDistance(device,onuid); err != nil {
