...
func (dh *DeviceHandler) doStateConnected(ctx context.Context) error {
	...
	dh.populateActivePorts(ports)
	if err := dh.disableAdminDownPorts(ports); err != nil {
...
func startCollector(dh *DeviceHandler) {
	...
	for {
		...
		case <-time.After(time.Duration(dh.metrics.ToPmConfigs().DefaultFreq) * time.Second):
			...
			if err != nil {
				logger.Warnw("failed-to-list-ports", log.Fields{"error": err})
				continue
			}
...
func (dh *DeviceHandler) DisableDevice(device *voltha.Device) error {
	...
	if err := dh.coreProxy.PortsStateUpdate(context.TODO(), cloned.Id, voltha.Port_PON_OLT, voltha.OperStatus_UNKNOWN); err != nil {
...
func (dh *DeviceHandler) ReenableDevice(device *voltha.Device) error {
	...
	ports, err := dh.coreProxy.ListDevicePorts(context.TODO(), device.Id)
	if err != nil {
...
func (dh *DeviceHandler) updateStateUnreachable(ctx context.Context) {
	...
	if device.ConnectStatus == voltha.ConnectStatus_REACHABLE {
		...
		if err = dh.coreProxy.PortsStateUpdate(ctx, dh.device.Id, voltha.Port_ALL, voltha.OperStatus_UNKNOWN); err != nil {
...
func (dh *DeviceHandler) disableAdminDownPorts(ports []*voltha.Port) error {
...
func (dh *DeviceHandler) populateActivePorts(ports []*voltha.Port) {
	logger.Infow("populateActiveports", log.Fields{"Device": dh.device.Id})
	for _, port := range ports {
		if port.Type == voltha.Port_ETHERNET_NNI {
