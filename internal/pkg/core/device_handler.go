...
func (dh *DeviceHandler) onuDiscIndication(ctx context.Context, onuDiscInd *oop.OnuDiscIndication) error {
	...
	if onuDevice == nil {
		...
		if onuDevice, error = dh.sendChildDeviceDetectedToCore(ctx, &ca.DeviceDiscovery{
			...
			if onuDevice != nil {
				...
				dh.sendFailureEvent(ctx, events.OnuUpdateCommunicationFailEvent, dh.device.Id, onuDevice.Id, onuDiscInd.GetIntfId(), 0, time.Now().Unix(), events.ErrCodeCoreUnavailable,
	...
	if error = dh.updateDeviceStateInCore(ctx, &ca.DeviceStateFilter{
		...
		dh.sendFailureEvent(ctx, events.OnuUpdateCommunicationFailEvent, dh.device.Id, onuDevice.Id, onuDiscInd.GetIntfId(), onuID, time.Now().Unix(), events.ErrCodeDeviceStateUpdateAtCore,
	...
	if error = dh.activateONU(ctx, onuDiscInd.IntfId, int64(onuID), onuDiscInd.SerialNumber, sn); error != nil {
		...
		dh.sendFailureEvent(ctx, events.OnuUpdateCommunicationFailEvent, dh.device.Id, onuDevice.Id, onuDiscInd.GetIntfId(), onuID, time.Now().Unix(), events.ErrCodeActivateOnu,
...
func (dh *DeviceHandler) onuIndication(ctx context.Context, onuInd *oop.OnuIndication) error {
	...
	if err != nil || onuDevice == nil {
		...
		dh.sendFailureEvent(ctx, events.OnuUpdateCommunicationFailEvent, dh.device.Id, devID, onuInd.GetIntfId(), onuInd.OnuId, time.Now().Unix(), events.ErrCodeCoreUnavailable, fmt.Sprintf("core not available  error: %v", err))
		return olterrors.NewErrNotFound("onu-device", errFields, err)
	}
	...
	if err := dh.updateOnuStates(ctx, onuDevice, onuInd); err != nil {
		...
		dh.sendFailureEvent(ctx, events.OnuUpdateCommunicationFailEvent, dh.device.Id, onuDevice.Id, onuInd.GetIntfId(), onuInd.OnuId, time.Now().Unix(), events.ErrCodeOnuStateUpdateAtAdapter, fmt.Sprintf("Onu state update at adapter failed  error: %v", err))
...
func (dh *DeviceHandler) cleanupDeviceResources(ctx context.Context) error {
	...
	if len(errs) == 0 {
		...
		if len(errs) > 0 {
			go dh.sendFailureEvent(ctx, events.DeviceDBUpdateFailureEvent, dh.device.Id, "", 0, 0, time.Now().Unix(), events.ErrCodeDeviceDbKvStoreUpdate, fmt.Sprintf("Failed to clean kv store for OLT device: %v", errs))
		}
...
func (dh *DeviceHandler) updateDeviceStateInCore(ctx context.Context, deviceStateFilter *ca.DeviceStateFilter) error {
	...
	if err != nil && deviceStateFilter.ParentDeviceId == "" { //ParentDeviceId checked to differentiate OLT or ONU event
		go dh.sendFailureEvent(ctx, events.DeviceStateUpdateFailedEvent, deviceStateFilter.DeviceId, "", 0, 0, time.Now().Unix(), events.ErrCodeDeviceStateUpdateAtCore,
			fmt.Sprintf("Device state update failed at rwcore: ConnStatus=%s, OperStatus=%s, error: %v",
...
func (dh *DeviceHandler) updateDeviceInCore(ctx context.Context, device *voltha.Device) error {
	...
	if err != nil {
		go dh.sendFailureEvent(ctx, events.DeviceStateUpdateFailedEvent, device.Id, "", 0, 0, time.Now().Unix(), events.ErrCodeDeviceStateUpdateAtCore,
...
func (dh *DeviceHandler) updateHeartbeatSignature(ctx context.Context, signature uint32) {
	...
	if err = dh.kvStore.Put(ctx, heartbeatPath, val); err != nil {
		...
		go dh.sendFailureEvent(ctx, events.DeviceDBUpdateFailureEvent, dh.device.Id, "", 0, 0, time.Now().Unix(), events.ErrCodeDeviceDbKvStoreUpdate, fmt.Sprintf("Failed to update heartbeat signature in KV store: %v", err))
	}
}
...
func (dh *DeviceHandler) sendFailureEvent(ctx context.Context, eventName string, deviceID, childDeviceID string, intfId, onuId uint32, raisedTs int64, errorCode events.FailureErrorCode, reason string) {
	...
	/* Send event to KAFKA */
	events.FailureEvent(ctx, dh.eventMgr.eventProxy, eventName, context, deviceID, raisedTs)
}
