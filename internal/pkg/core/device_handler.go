...
func (dh *DeviceHandler) onuDiscIndication(ctx context.Context, onuDiscInd *oop.OnuDiscIndication) error {
	...
	subCtx, cancel := context.WithTimeout(log.WithSpanFromContext(context.Background(), ctx), dh.cfg.RPCTimeout)
	err = dh.setupChildInterAdapterClient(subCtx, onuDevice)
	cancel()
	if err != nil {
...
// supports is deleted
func (dh *DeviceHandler) setupChildInterAdapterClient(ctx context.Context, childDevice *voltha.Device) error {
	dh.lockChildAdapterClients.Lock()
	defer dh.lockChildAdapterClients.Unlock()
	if _, ok := dh.childAdapterClients[childDevice.AdapterEndpoint]; ok {
		// Already set
		return nil
	}
	...
	var err error
	if dh.childAdapterClients[childDevice.AdapterEndpoint], err = vgrpc.NewClient(childDevice.AdapterEndpoint,
		dh.onuAdapterRestarted,
		vgrpc.ActivityCheck(true),
		vgrpc.ActivityProbeInterval(dh.cfg.LiveProbeInterval)); err != nil {
		logger.Errorw(ctx, "grpc-client-not-created", log.Fields{"error": err, "endpoint": childDevice.AdapterEndpoint})
		return err
	}
	go dh.childAdapterClients[childDevice.AdapterEndpoint].Start(log.WithSpanFromContext(context.TODO(), ctx), setAndTestAdapterServiceHandler)
	...
	for {
		client, err := dh.childAdapterClients[childDevice.AdapterEndpoint].GetOnuInterAdapterServiceClient()
		if err == nil && client != nil {
			logger.Infow(subCtx, "connected-to-child-adapter", log.Fields{"child-endpoint": childDevice.AdapterEndpoint})
			break
		}
		logger.Warnw(subCtx, "connection-to-child-adapter-not-ready", log.Fields{"error": err, "child-endpoint": childDevice.AdapterEndpoint})
		// Backoff
		if err = backoff.Backoff(subCtx); err != nil {
			logger.Errorw(subCtx, "received-error-on-backoff", log.Fields{"error": err, "child-endpoint": childDevice.AdapterEndpoint})
			break
		}
	}
	return nil
...
func (dh *DeviceHandler) onuAdapterRestarted(ctx context.Context, endPoint string) error {
	logger.Errorw(ctx, "onu-adapter-restarted", log.Fields{"endpoint": endPoint})
	return nil
}
