...
func (dh *DeviceHandler) getPONRxPower(ctx context.Context, OltRxPowerRequest *extension.GetOltRxPowerRequest) *extension.SingleGetValueResponse {
	...
	if serialNumber != "" {
		...
		if onuDev != nil {
			...
			logger.Infow(ctx, "getPONRxPower response ", log.Fields{"oltPortRxPower": rxPowerValue})
			logger.Infow(ctx, "getPONRxPower response ", log.Fields{"oltPortRxPowerValue": rxPowerValue.RxPower})
