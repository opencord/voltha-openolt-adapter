...
func (dh *DeviceHandler) getExtValue(device *voltha.Device, onuid string, value voltha.ValueType_Type) (*voltha.ReturnValues, error) {
	var err error
	var ID uint32
	resp := new(voltha.ReturnValues)
	valueparam := new(oop.ValueParam)
	ctx := context.Background()
	log.Infow("getValue", log.Fields{"onu-id": onuid, "pon-intf": device.ParentPortNo})
	sn := new(oop.SerialNumber)
	if sn, err = dh.deStringifySerialNumber(device.SerialNumber); err != nil {
		log.Errorw("error-while-deStringifySerialNumber", log.Fields{"DeviceID": dh.device, "onu-id": onuid, "error": err})
		return nil, err
	}
	...
	*/
	...
	log.Infow("get-onu-distance", log.Fields{"resp": resp, "DeviceID": dh.device, "onuid": onuid, "pon-intf": device.ParentPortNo})
	return resp, nil
}
