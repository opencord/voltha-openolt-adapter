...
func (dh *DeviceHandler) initializeDeviceHandlerModules(ctx context.Context) error {
	...
	for i = 0; i < dh.totalPonPorts; i++ {
		...
		if dh.resourceMgr[i] = rsrcMgr.NewResourceMgr(ctx, i, dh.device.Id, dh.openOLT.KVStoreAddress, dh.openOLT.KVStoreType, dh.device.Type, deviceInfo, dh.cm.Backend.PathPrefix); dh.resourceMgr[i] == nil {
...
func (dh *DeviceHandler) cleanupDeviceResources(ctx context.Context) {
	...
	if dh.resourceMgr != nil {
		...
		for ponPort = 0; ponPort < dh.totalPonPorts; ponPort++ {
			...
			err := dh.resourceMgr[ponPort].PonRsrMgr.GetOnuGemInfo(ctx, ponPort, &onuGemData)
			if err != nil {
			...
			err = dh.resourceMgr[ponPort].DelOnuGemInfoForIntf(ctx, ponPort)
			if err != nil {
		...
		go func() {
			if err := dh.resourceMgr[ponPort].Delete(ctx); err != nil {
...
func (dh *DeviceHandler) ChildDeviceLost(ctx context.Context, pPortNo uint32, onuID uint32, onuSn string) error {
	...
	var onuGemData []rsrcMgr.OnuGemInfo
	if ponRsrMgr := dh.resourceMgr[intfID].PonRsrMgr; ponRsrMgr == nil {
		logger.Warnw(ctx, "failed-to-get-resource-manager-for-interface-Id", log.Fields{
			"device-id": dh.device.Id,
			"intf-id":   intfID})
	} else {
		if err := ponRsrMgr.GetOnuGemInfo(ctx, intfID, &onuGemData); err != nil {
			...
			for i, onu := range onuGemData {
				if onu.OnuID == onuID && onu.SerialNumber == onuSn {
					...
					err := ponRsrMgr.AddOnuGemInfo(ctx, intfID, onuGemData)
					if err != nil {
					...
					dh.resourceMgr[intfID].FreeonuID(ctx, intfID, []uint32{onu.OnuID})
					break
				}
			}
		}
