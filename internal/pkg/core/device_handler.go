...
// ProxyOnuSwDownloadSectionsOmciMessage sends the proxied OMCI message to the target device
func (dh *DeviceHandler) ProxyOnuSwDownloadSectionsOmciMessage(ctx context.Context, omciMsg *ia.OnuSwDownloadOmciMessage) error {
	if omciMsg.GetProxyAddress() == nil {
		...
		if err := dh.sendOnuSwDownloadSectionsProxiedMessage(log.WithSpanFromContext(context.Background(), ctx), onuDevice, omciMsg); err != nil {
		...
		if err := dh.sendOnuSwDownloadSectionsProxiedMessage(log.WithSpanFromContext(context.Background(), ctx), nil, omciMsg); err != nil {
			return olterrors.NewErrCommunication("send-failed", log.Fields{
				"parent-device-id": dh.device.Id,
				"child-device-id":  omciMsg.ChildDeviceId}, err)
		}
...
func (dh *DeviceHandler) sendOnuSwDownloadSectionsProxiedMessage(ctx context.Context, onuDevice *voltha.Device, omciMsg *ia.OnuSwDownloadOmciMessage) error {
	var intfID uint32
	var onuID uint32
	...
	onuSecOmciMsgList := omciMsg.GetMessage()
