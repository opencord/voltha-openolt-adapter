...
func (dh *DeviceHandler) onuDiscIndication(ctx context.Context, onuDiscInd *oop.OnuDiscIndication) error {
	...
	raisedTs := time.Now().Unix()
	if processed, loaded := dh.discOnus.LoadOrStore(sn, true); loaded {
		if processed.(bool) {
			...
			logger.Infow(ctx, "onu-processing-had-completed-but-new-indication", log.Fields{"sn": sn})
	...
	defer func() {
		...
		// we know the processing has completed.
	...
	// here is where it implodes, look into the core
	if err := dh.updateDeviceStateInCore(ctx, &ca.DeviceStateFilter{
		DeviceId:       onuDevice.Id,
