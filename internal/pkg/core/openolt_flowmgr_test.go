...
func TestOpenOltFlowMgr_checkAndAddFlow(t *testing.T) {
	...
	//}
/*
	uniPortName := "olt-{1234}/pon-{1}/onu-{1}/uni-{0}"
	TpInst, err := flowMgr[1].techprofile[1].CreateTechProfInstance(nil, 64, uniPortName, 1)
	if err != nil {
		// Error logging is already done in the called function
		// So just return in case of error
		return
	}
	xgPonInst := TpInst.(*tp.TechProfile)
	xgPonInst.UsScheduler.Priority = 1
	xgPonInst.UsScheduler.Direction = "upstream"
	xgPonInst.UsScheduler.AllocID = 1
	xgPonInst.UsScheduler.AdditionalBw = "None"
	xgPonInst.UsScheduler.QSchedPolicy = "PQ"
	xgPonInst.UsScheduler.Weight = 4

	xgPonInst.DsScheduler.Priority = 1
	xgPonInst.DsScheduler.Direction = "upstream"
	xgPonInst.DsScheduler.AllocID = 1
	xgPonInst.DsScheduler.AdditionalBw = "None"
	xgPonInst.DsScheduler.QSchedPolicy = "PQ"
	xgPonInst.DsScheduler.Weight = 4

	xgPonInst = append(xgPonInst.UpstreamGemPortAttributeList, )
