...
func TestOpenOltFlowMgr_addGemPortToOnuInfoMap(t *testing.T) {
	...
	for i := 0; i < intfNum; i++ {
		assert.Equal(t, onuNum, len(flowMgr.onuGemInfo[uint32(i)]))

		for o := 0; o < onuNum; o++ {
			// check that each onuEntry has 1 gemPort
			assert.Equal(t, 1, len(flowMgr.onuGemInfo[uint32(i)][o].GemPorts))

			// check that the value of the gemport is correct
			gemId, _ := strconv.Atoi(fmt.Sprintf("90%d%d", i, o))
			assert.Equal(t, uint32(gemId), flowMgr.onuGemInfo[uint32(i)][o].GemPorts[0])
		}
	}
}
...
func TestOpenOltFlowMgr_deleteGemPortFromLocalCache(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.args.gemPortIDsRemaining, flowMgr.onuGemInfo[tt.args.intfID][0].GemPorts)
