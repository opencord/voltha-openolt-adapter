...
const (
	...
	base10 = 10
	// onuRemoteDefectEventThreshold is the threshold value for ONU RDI event
	defaultONURemoteDefectEventThreshold = 3
)
...
func (em *OpenOltEventMgr) onuRemoteDefectIndication(onuID uint32, intfID uint32, rdiErrors uint64, status string, deviceID string, raisedTs int64) error {
	/* Populating event context */
	context := map[string]string{
		"onu-id":     strconv.FormatUint(uint64(onuID), base10),
		"intf-id":    strconv.FormatUint(uint64(intfID), base10),
		"rdi-errors": strconv.FormatUint(rdiErrors, base10),
	}
	/* Populating device event body */
...
func (em *OpenOltEventMgr) onuItuPonStatsIndication(onuIPS *oop.OnuItuPonStatsIndication, deviceID string, raisedTs int64) error {
	...
	if onuIPS.RdiErrors > onuDevice.(*OnuDevice).rdThreshold {
