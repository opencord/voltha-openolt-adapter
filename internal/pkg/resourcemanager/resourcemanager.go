...
func (rsrcMgr *OpenOltResourceMgr) GetFlowIDsForGem(ctx context.Context, gem uint32) ([]uint64, error) {
	intfID := rsrcMgr.PonIntfID
	// The path for NNI trap flows should look like this: flowids_per_gem/{4294967295}/{4294967295}
	if gem == NNI {
		intfID = NNI
	}
	path := fmt.Sprintf(FlowIDsForGem, intfID, gem)
